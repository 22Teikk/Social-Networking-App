...
class ImageStoryAdapter(
    ...
    override fun instantiateItem(container: ViewGroup, position: Int): Any {
        ...
        binding.delStory.setOnClickListener {
            Firebase.database.reference.child(Constant.STORY_TABLE_NAME).child(storyList[position].uid.toString()).child(storyList[position].storyID.toString()).removeValue()
    ...
    private fun getCountViewStory(binding: ImageStoryItemBinding, storyID: String, auth: String) {
        ...
        Firebase.database.reference.child(Constant.STORY_TABLE_NAME).child(auth).child(storyID)
            .child("viewer")
